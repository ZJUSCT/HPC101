...
=======
从 Mixtral 到 DeepSeek-V3 与 Qwen，**MoE（Mixture of Experts，混合专家）**已经成为大模型的主流架构。本次实验将带你实现并优化一个 DeepSeek-V3 风格的量化 MoE 层前向计算，在这个过程中：
...
=======
本实验采用 [DeepSeek-V3](https://arxiv.org/abs/2412.19437) 的 MoE 架构（DeepSeekMoE），它与早期 MoE（如 Mixtral 的 8 专家选 2）相比有几个标志性设计：

- **细粒度专家（fine-grained experts）**：专家数量更多、单个专家更小（V3 是 256 个路由专家选 8 个）。同样的总参数量下，专家组合的可能性大大增加；
- **共享专家（shared expert）**：一个所有 token 都必须经过的专家，负责学习通用知识，让路由专家专注于特化知识；
- **Sigmoid 亲和度 + 无辅助损失的负载均衡**：router 用 sigmoid（而不是 softmax）计算每个专家的亲和度分数；每个专家带一个偏置 $b_e$，它**只参与 Top-K 的选择、不参与 gate 权重的计算**。训练时动态调整这些偏置就能均衡各专家的负载，而无需在损失函数中加入辅助项（auxiliary-loss-free load balancing）。推理时这些偏置是从 checkpoint 加载的常量。

本实验的问题规模是 DeepSeek-V3 的等比缩小版：**16 个路由专家选 4 个，外加 1 个共享专家**。
>>>>>>> 0e1a6fe (feat(lab2): adopt DeepSeek-V3 MoE architecture and student-file framework)
...
=======
flowchart LR
    X["token 向量 x (fp32)"] --> R["Router (fp32)<br>sigmoid 亲和度<br>+偏置选 Top-4"]
    X --> Q["激活量化<br>fp32 → int8"]
    Q --> SE["共享专家 (int8)<br>SwiGLU FFN"]
    Q --> E["路由专家 × 4 (int8)<br>SwiGLU FFN"]
    R -- "gate 权重" --> C["加权合并<br>+ 残差 (fp32)"]
    SE --> C
    E --> C
    C --> Y["输出 y (fp32)"]
>>>>>>> 0e1a6fe (feat(lab2): adopt DeepSeek-V3 MoE architecture and student-file framework)
...
$$
=======
1. **亲和度**：router 给每个路由专家打分：$s_e = \sigma(w_e^\top x_t)$，$\sigma$ 为 sigmoid；
2. **Top-K 选择**：按**带偏置的分数** $s_e + b_e$ 选出前 $K$ 个专家（偏置只在这一步出现）；
3. **Gate 权重**：被选中专家的**原始亲和度**归一化：$g_e = s_e \big/ \sum_{e' \in \text{TopK}} s_{e'}$；
4. **专家 FFN**：共享专家和每个被选中的路由专家都是 SwiGLU 结构：

    $$
    \text{FFN}(x) = W_{down} \big( \mathrm{SiLU}(W_{gate}\, x) \odot (W_{up}\, x) \big), \quad \mathrm{SiLU}(v) = \frac{v}{1 + e^{-v}}
    $$

    其中 $W_{gate}, W_{up} \in \mathbb{R}^{d_{ff} \times d_{model}}$，$W_{down} \in \mathbb{R}^{d_{model} \times d_{ff}}$，$\odot$ 是逐元素相乘；

5. **合并**：$y_t = x_t + \text{FFN}_{shared}(x_t) + \sum_{e \in \text{TopK}} g_e \cdot \text{FFN}_e(x_t)$（含残差连接）。
>>>>>>> 0e1a6fe (feat(lab2): adopt DeepSeek-V3 MoE architecture and student-file framework)
...
=======
- **激活**使用 per-token scale：每个 token 向量单独计算 $s_x$（激活的动态范围逐 token 变化很大）；
- **权重**每个矩阵一个 scale（如 $W_{gate}^{(e)}$ 共享一个 $s_{gate}^{(e)}$；真实引擎通常做到 per-channel，这里为了简化用 per-tensor）；
- int8 × int8 的点积在 **int32 中累加**（想一想为什么不能在 int8/int16 中累加），随后乘以 scale 反量化回 fp32：
>>>>>>> 0e1a6fe (feat(lab2): adopt DeepSeek-V3 MoE architecture and student-file framework)
...
| 阶段 | 计算 | 精度 |
| ---- | ---- | ---- |
| 1. 亲和度 | $s_e = \sigma(w_e^\top x_t)$ | fp32 |
| 2. Top-K | 按 $s_e + b_e$ 选出 $K$ 个专家 | fp32 |
| 3. Gate | $g_e = s_e / \sum_{\text{TopK}} s$（不含偏置） | fp32 |
| 4. 激活量化 | $x_q = \mathrm{round}(x_t / s_x)$ | fp32 → int8 |
| 5a. gate/up 投影 | $h_g = (W_{gate} x_q) s_x s_{gate}$，$h_u = (W_{up} x_q) s_x s_{up}$ | int8 点积 → int32 → fp32 |
| 5b. SwiGLU | $h = \mathrm{SiLU}(h_g) \odot h_u$ | fp32 |
| 5c. 重量化 | $h_q = \mathrm{round}(h / s_h)$，$s_h = \max\|h\| / 127$ | fp32 → int8 |
| 5d. down 投影 | $o = (W_{down} h_q) \cdot s_h s_{down}$ | int8 点积 → int32 → fp32 |
| 6. 合并 | $y_t = x_t + o_{shared} + \sum g_e \cdot o_e$ | fp32 |
>>>>>>> 0e1a6fe (feat(lab2): adopt DeepSeek-V3 MoE architecture and student-file framework)
...
```text
src/lab2
├── CMakeLists.txt        # 两个构建目标：lab2（主任务）、lab2_bonus（Bonus）
├── main.cpp              # 主任务 driver（评测时替换为原版）
├── bonus.cpp             # Bonus driver（评测时替换为原版）
├── include
│   ├── moe.h             # 问题规模、权重结构体、接口声明
│   └── lora.h            # Bonus：LoRA 适配器与接口声明
├── src                   # 框架代码（评测时替换为原版）
│   ├── moe_ref.cpp       # 主任务的标量参考实现（正确性基准）
│   ├── lora_ref.cpp      # Bonus：带 LoRA 旁路的参考前向
│   ├── gradcheck.cpp     # Bonus：有限差分梯度校验器
│   └── data.cpp          # 数据初始化与正确性检查
└── student               # ★ 你的代码（只有这个目录会被收取）
    ├── moe_opt.cpp       # 主任务：preprocess + moe_forward_optimized
    └── lora_opt.cpp      # Bonus：LoRA 前向优化 + 反向
```

问题规模定义在 `include/moe.h` 中：128 个 token，$d_{model} = 256$，$d_{ff} = 128$（细粒度专家），16 个路由专家选 4 个 + 1 个共享专家。

!!! danger "修改范围限制"

    **你只能修改 `student/` 目录下的两个文件**。评测时，`student/` 之外的所有文件
    （driver、头文件、参考实现、CMakeLists）都会被替换为原版——所以就算你在别处改了什么，
    评测时也不会生效。你可以在自己的文件里自由添加辅助函数、头文件引用、全局缓冲区等，
    只要实现声明在 `moe.h` / `lora.h` 中的接口函数即可。

`student/moe_opt.cpp` 需要实现两个函数：

```cpp linenums="1"
// 计时开始前调用一次。你可以在这里对权重做重排/预打包（真实推理引擎
// 都会离线做 weight pre-packing）。这里看不到输入 token —— 不允许
// 预计算任何答案。
void preprocess(MoEWeights& w) {}

// 你的优化实现。默认直接调用参考实现（Speedup ≈ 1.0），把它换掉。
...
=======
    - **Baseline 的访存模式好吗？** 参考实现逐 token 计算，每个 token 都要完整读一遍它所选专家的
      全部权重（每个路由专家 3 个矩阵共 96 KiB）。热点在哪里、瓶颈是访存还是计算，建议先估算再动手（见思考题 1）。
    - **真实的 MoE 推理引擎是怎么做的？** 先把 token 按专家**分组**（dispatch），再对每个专家把它
      名下的所有 token 一起算（矩阵-向量乘变成矩阵-矩阵乘），最后把结果散射回去（combine）。
      权重从「每 token 读一遍」变成「每批读一遍」。注意共享专家天然就是全批 token 的矩阵乘。
    - **数据布局**：`preprocess` 允许你把权重重排成对向量指令友好的布局。
    - **逐阶段向量化**：router 的 fp32 矩阵乘、sigmoid/SiLU、量化、int8 点积……每一段都可以向量化，
      但收益差别巨大——用数据说话，把时间花在热点上。
>>>>>>> 0e1a6fe (feat(lab2): adopt DeepSeek-V3 MoE architecture and student-file framework)
...
=======
训练时只更新 $A$ 和 $B$——可训练参数量骤降几个数量级。[QLoRA](https://arxiv.org/abs/2305.14314) 更进一步：底座权重以量化形式冻结（省显存），fp32/bf16 的低秩旁路负责学习。这正是本 Bonus 的设定：**int8 底座 + fp32 LoRA 旁路**。

### 本实验的设定

每个**路由专家**的 down 投影挂一个 rank-8 的 fp32 适配器（共享专家不挂），旁路从重量化**之前**的 fp32 隐层激活 $h$（SwiGLU 的输出）读入：

$$
o_e = \underbrace{(W_{down}^{(e)} h_q) \cdot s_h s_{down}^{(e)}}_{\text{int8 底座（冻结）}} + \underbrace{B^{(e)} (A^{(e)} h)}_{\text{fp32 旁路（可训练）}}
$$

其余部分（router、量化、gate/up 投影、gate 合并、残差）与主任务完全相同。参考实现在 `src/lora_ref.cpp` 中，它就是你的反向传播需要匹配的精确规格。

### 任务

`student/lora_opt.cpp` 需要实现两个函数：

1. **前向**（`moe_lora_forward_optimized`）：实现带旁路的前向并优化。默认调用参考实现，评分规则与主任务相同。顺便体会一下：瘦长矩阵乘（$8 \times 128$ 的 $A$）和方矩阵乘的优化手感有什么不同？
2. **反向**（`moe_lora_backward`）：给定上游梯度 $g_t = \partial L / \partial y_t$，推导并实现适配器梯度 $\partial L / \partial A^{(e)}$ 和 $\partial L / \partial B^{(e)}$。底座权重全部冻结，**不需要**计算 $dW$ 或 $dx$。

!!! tip "推导提示"

    - 损失定义为 $L = \sum_t g_t^\top y_t$，它对 $(A, B)$ 是双线性的——链式法则一步就能走通；
    - 只有被路由到专家 $e$ 的 token 才对 $A^{(e)}, B^{(e)}$ 的梯度有贡献，别忘了 gate 权重；
    - 反向需要前向的中间量（路由结果、$h$、$A h$），在反向函数里重新计算它们即可。

### 正确性验证

harness 会用**中心有限差分**自动校验你的梯度：随机抽取 32 个适配器坐标，逐个扰动后重跑参考前向，
把数值梯度和你的解析梯度做比对。全部通过才算正确，随后会单独给你的反向计时：

```text
Gradient check passed! (32 sampled coordinates, max error: ...)
Backward time: ... s
>>>>>>> 0e1a6fe (feat(lab2): adopt DeepSeek-V3 MoE architecture and student-file framework)
```
...
1. 使用GitLab账号登录https://platform.s.zjusct.io/
2. 点击 `创建第一个` 或者右上角 `New +` 添加新的环境
3. 按照你的喜好随意填写一个名称。若是要完成基础任务，请进一步点击x86-5418Y; 若是要完成bonus任务，请进一步点击riscv-K1
4. 等待状态从 `Pending` 转换成 `Running` 之后，便可直接通过复制 `ssh -p 443 h3240104810+new+hpc101@clusters.zju.edu.cn`到本地终端直接登录，或者通过配置SSH config来通过vscode远程连接
...
### 主线任务
=======
1. **算术强度**：估算参考实现一次前向的总访存量（提示：专家权重被读了多少遍？）和总乘加次数，
   计算算术强度（MACs/byte）。按专家分组之后这两个数字分别变成多少？由此说明这个负载是
   访存瓶颈还是计算瓶颈，以及分组为什么能加速。
2. **负载均衡偏置**：偏置 $b_e$ 为什么只参与 Top-K 选择、不参与 gate 权重的计算？
   结合「训练时上调低负载专家的偏置、下调高负载专家的偏置」这一规则，
   说明这个设计如何在不影响模型输出质量的前提下均衡专家负载。
3. **量化设计**：为什么激活用 per-token scale，而权重每个矩阵一个 scale 就够了？
   如果让一批 128 个 token 共享同一个激活 scale，会发生什么？
4. **累加精度**：int8 × int8 的点积为什么必须在 int32 中累加？以本实验的 $d_{model}$ 和 $d_{ff}$
   计算，最坏情况下的累加值是多少？
5. **（Bonus）旁路的位置**：为什么 LoRA 旁路要从重量化之前的 fp32 $h$ 读入，而不是从 $h_q$？
   如果从 $h_q$ 读入，有限差分校验会发生什么？（提示：$\mathrm{round}$ 的导数是什么？）
6. **（Bonus）有限差分**：`gradcheck.cpp` 用了大到 $\varepsilon = 0.1$ 的差分步长——通常数值微分
   的步长要取得很小，为什么这里反而可以（甚至应该）取这么大？

## 实验任务与要求

1. 完成 `student/moe_opt.cpp` 中 MoE 前向的优化（主任务）；
2. Bonus：完成 `student/lora_opt.cpp` 中带 LoRA 旁路的前向优化与反向实现；
3. 提交到学在浙大：
    1. 代码（`student/` 目录下的文件）；
    2. 实验报告，包含：
        1. 优化思路（做了哪些优化、为什么、各带来多少收益）；
        2. 正确性与加速比的运行截图；
        3. 思考题解答；
        4. （Bonus）梯度公式的推导过程。
>>>>>>> 0e1a6fe (feat(lab2): adopt DeepSeek-V3 MoE architecture and student-file framework)
...
=======
    本次实验的 OJ 评测方式将在实验发布前更新，请关注群内通知。
    评测时只会收取 `student/` 目录，其余文件一律替换为原版。

## 参考资料

- [Mixture of Experts Explained](https://huggingface.co/blog/moe)：Hugging Face 的 MoE 科普长文
- [DeepSeek-V3 Technical Report](https://arxiv.org/abs/2412.19437)：本实验架构的出处（§2.1.2 DeepSeekMoE）
- [DeepSeekMoE: Towards Ultimate Expert Specialization](https://arxiv.org/abs/2401.06066)：细粒度专家与共享专家的设计动机
- [Auxiliary-Loss-Free Load Balancing](https://arxiv.org/abs/2408.15664)：无辅助损失负载均衡策略
- [LoRA: Low-Rank Adaptation of Large Language Models](https://arxiv.org/abs/2106.09685)
- [QLoRA: Efficient Finetuning of Quantized LLMs](https://arxiv.org/abs/2305.14314)
- [What Every Programmer Should Know About Memory](https://people.freebsd.org/~lstewart/articles/cpumemory.pdf)：理解访存瓶颈的经典材料
>>>>>>> 0e1a6fe (feat(lab2): adopt DeepSeek-V3 MoE architecture and student-file framework)
