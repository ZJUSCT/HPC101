...
!!! warning "本节内容尚未定稿😭"
    ...
    代码框架依赖实验平台与机器搭建工作，目前仍在调整中，与你后续实际拿到的环境可能不一致。我们会在机器和实验环境敲定后再次修订本节；请及时关注课程群通知，耐心等待。
...
本实验通过课程平台提供的容器运行。平台会根据所在节点架构自动选择匹配的 child image；你**不需要**自己拉镜像或选架构。

| 节点架构 | 硬件 | 默认工具链 | 构建产物 |
| -------- | ---- | ---------- | -------- |
| `linux/arm64` | 华为鲲鹏 920B | GNU 14 + OpenMPI 5，`AMSS_ENABLE_GPU=OFF` | `TwoPunctureABE`、`ABE` |
| `linux/amd64` | x86_64 + NVIDIA V100 | GNU 13 + OpenMPI 5 + Debian CUDA 12.4，`sm_70` | `TwoPunctureABE`、`ABE`、`ABEGPU` |

镜像只包含开发环境，**不复制** Lab 4 源码。你可以在容器内任意路径 clone HPC101-Internal，或者使用平台已经预挂载到某绝对路径的 checkout。镜像的入口位于 `/opt/hpc101/lab4/`，由 `entrypoint` 加载 `activate` 后执行你的命令；非交互和交互 shell 看到的环境一致。

#### 默认环境

- ARM 镜像默认：GNU 14 + GFortran 14 + OpenMPI 5，`AMSS_ENABLE_GPU=OFF`。
- x86 镜像默认：GNU 13 + GFortran 13 + OpenMPI 5 + Debian CUDA 12.4（`sm_70`），`AMSS_ENABLE_GPU=ON`，`CUDAHOSTCXX=g++-13`，`CUDACXX=/usr/bin/nvcc`。

!!! warning "x86/V100 使用 CUDA 12.4，不是 CUDA 13"

    课程 x86 镜像固定使用 Debian 13 自带的 `nvidia-cuda-toolkit 12.4.1`，**不**使用 CUDA 13。
    CUDA 13 已经删除 `sm_70`（V100）支持；在 V100 上必须使用 CUDA 12.4。
    修改 `CMAKE_CUDA_ARCHITECTURES` 时务必保持 `70`；不要把它改成 A100 对应的 `80` 或更新架构。

    NVIDIA kernel driver 不在镜像内，由宿主机通过 NVIDIA Container Toolkit 注入。运行前请确认 V100 节点 driver 满足 CUDA 12.x minor compatibility floor（≥ `525.60.13`）。

#### 四个目录变量

`compile.sh`、`run.sh`、`check.sh` 和 `AMSS_NCKU_Program.py` 都遵守同一组环境变量。相对路径以 Lab 4 root 为基准解析，而不是当前工作目录。

| 变量 | 默认值 | 用途 |
| ---- | ------ | ---- |
| `AMSS_BUILD_DIR` | `<lab4>/build` | CMake 构建产物。`TwoPunctureABE` / `ABE` / `ABEGPU` 落在这里。 |
| `AMSS_OUTPUT_ROOT` | `<lab4>` | `File_directory` 的父目录。 |
| `AMSS_CACHE_DIR` | `<lab4>/twopuncture_cache` | `--twop-cache` 缓存目录。 |
| `AMSS_MPIEXEC` | `mpiexec` | MPI launcher。用 `shlex.split` 解析，可带 flag。 |

`AMSS_NCKU_Input.py` 的 `File_directory` 可写绝对路径或相对路径；相对路径会拼接 `AMSS_OUTPUT_ROOT`。driver 会把解析后的绝对路径回写，helper scripts 不需要任何改动就能看到正确位置。

#### 默认 OpenMPI，可切到 MPICH

默认使用 OpenMPI 5。如果你需要对比 MPICH 4：

```bash
source /opt/hpc101/lab4/envs/mpich.sh
rm -rf $AMSS_BUILD_DIR            # CMake cache 记录了 OpenMPI 的 wrapper 路径，必须换 build 目录
./compile.sh
```

切换 MPI 实现**必须**使用新的 `AMSS_BUILD_DIR`。直接复用旧 cache 会被 `compile.sh` 拒绝。

#### CUDA-aware MPI 默认关闭

`AMSS_MPI_CUDA_AWARE` 默认是 `0`。Debian 13 自带的 OpenMPI/MPICH 不宣称 CUDA-aware 支持，AMSS 当前代码路径使用 host staging。若想实验 device buffer 直接通信，需要先修改 `macrodef.h` 或在 `compile.sh` 中传入 `-DAMSS_MPI_CUDA_AWARE=1`，并确认你的 MPI 实现实际支持 CUDA buffer。

#### UID/GID

如果平台分配给你的容器 UID/GID 与宿主文件 owner 不同，请确认 build/output/cache 目录对当前 UID 可写。entrypoint 不会 `chown` 任何文件。
-->
...
<!-- !!! warning "容器中使用 perf 的权限"

    `perf` 需要宿主机暴露 `perf_event_open`。课程平台会在 ARM 容器中授予受限的 perf capability；如果运行时报 `perf_event_paranoid` 权限不足，应联系助教确认平台 perf 配置，而不是跳过 profiling。`linux-perf` 包已经预装在 ARM 镜像中。 -->
...
!!! info "VTune 与 Nsight 的容器权限"

    VTune 是 Intel 商业软件，课程平台需要先确认再分发许可，再决定是内置到 x86 镜像还是从宿主只读挂载。如果你在容器里 `source /opt/hpc101/lab4/envs/vtune.sh` 时看到 `VTUNE_HOME is not set` 提示，说明当前平台选择了挂载方式且未配置 `VTUNE_HOME`；请联系助教获取正确的 VTune 入口。

    Nsight Systems 和 Nsight Compute 已经预装在 x86 镜像中（`nsys`、`ncu`），可以直接调用。它们对 CUDA driver 和 `/proc` 可见性有要求；如果 `nsys profile` 报权限错误，应联系助教确认 NVIDIA Container Toolkit 是否注入了完整的 driver/device 权限。
...
!!! note "关于编译参数"
    ...
    当前 `CMakeLists.txt` 中的 `AMSS_OPT` 只作用于 C++ 和 Fortran，CUDA 架构与 NVCC 参数需要单独设置。我们已经把 `CMAKE_CUDA_ARCHITECTURES` 固定为 `70`（V100 `sm_70`）；
    
    任务一中 `AMSS_ENABLE_GPU=OFF`，CMake 不会启用 CUDA 语言。请在完成任务二时自行开启。
...
这些参数会通过 `run.sh` / `scripts/makefile_and_run.py` 传递给运行程序。
