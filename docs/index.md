...
!!! warning "网络与 DNS 配置"
    ...
    - **在校园网内**（如连接 ZJUWLAN 或紫金港有线网络）时，请确保使用**校园 DNS**（即校园网通过 DHCP 自动下发的 DNS 服务器），而不要手动改用公共 DNS（如 `8.8.8.8`、`114.114.114.114`），否则部分校内服务域名可能解析错误。
    - **使用 VPN / 代理时**，请注意其 DNS 解析行为。若代理把域名解析到了非校园地址，或使用公共 DNS 解析校园内网域名，会导致访问失败。
...
课程当前版本为 `main` 分支，历史版本保存于仓库的不同分支中，因此仓库总体体积较大。为了加快速度，你可以在克隆时指定单个分支：
...
MkDocs 对 Markdown 有一些增强要求，例如要求多级列表逐级缩进为 4 个空格等。为了使仓库内的代码符合相关工具的规范，我们使用 Linter 工具和插件进行格式化，仓库内包含相关配置文件护建议文档维护者和贡献者使用：

| 代码种类 | 工具 | 配置文件 |
| -------- | ---- | -------- |
| Markdown | VSCode 插件 [markdownlint](https://marketplace.visualstudio.com/items?itemName=DavidAnson.vscode-markdownlint) | `.vscode/settings.json' |
...
```text
├── slides：MkSlides 制作的课程 PPT
└── beamer：Latex Beamer 制作的课程 PPT
    ├── Makefile
    ├── day3.tex：PPT 主 Tex 文档
    ├── day3
    │   ├── img：图片资源
    │   └── sections：章节文档
    └── zjusct-beamer：ZJUSCT Beamer 主题
```

- LaTex Beamer：
    - 构建：目标为对应的文件名，例如 `make day1.tex`。构建后会在 `.output` 目录下生成 PDF 文件。
    - 在提交前，请运行 `make lint && make clean`。不要把构建完成的 PDF 文件提交到仓库中。
- [MartenBE/mkslides](https://github.com/MartenBE/mkslides/)：
    - 构建：由 GitHub Action 自动执行，集成到 MkDocs 网页中展示。
    - 参考了下列样式：
